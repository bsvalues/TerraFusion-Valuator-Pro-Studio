...
import { blankWorkfile, WorkfileDef } from "@/lib/workfile-types";
...
function WorkbenchInner() {
  ...
  const urlWorkfileId = searchParams.get("id");
  const urlOrderId = searchParams.get("order");

  const [wf, setWf] = useState<WorkfileDef>(() => blankWorkfile());
  ...
  // Load existing workfile when ?id= is present
  useEffect(() => {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (workfileId) {
        ...
        setWorkfileId(newId);
        // Link the workfile back to the order if one was provided
        if (orderId) {
          await updateOrder(orderId, { workfile_id: newId, status: "in-progress" }).catch(() => {});
        }
        // Update URL without full reload so future saves use PUT
        window.history.replaceState({}, "", `/?id=${newId}${orderId ? `&order=${orderId}` : ""}`);
      }
      void record;
      setSaveState("saved");
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
      ...
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-2 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-slate-400">FNMA 1004 — Uniform Residential Appraisal Report</span>
        </div>
        <div className="flex items-center gap-3">
          {workfileId && (
          ...
          {workfileId && (
            <a
              href={`/api/workfiles/${workfileId}/mismo`}
              download
              className="rounded px-3 py-1 text-xs font-semibold bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 transition-colors"
            >
              MISMO XML
            </a>
          )}
