...
import { blank2055, WorkfileDef } from "@/lib/workfile-types";
...
function WorkbenchInner() {
  ...
  const [wf, setWf] = useState<WorkfileDef>(() => blank2055());
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (workfileId) {
        ...
        window.history.replaceState({}, "", `/2055?id=${newId}${orderId ? `&order=${orderId}` : ""}`);
      }
      void record;
      setSaveState("saved");
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-2 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-slate-400">FNMA 2055 — Exterior-Only Inspection Residential Appraisal Report</span>
        </div>
        <div className="flex items-center gap-3">
          {workfileId && (
...
export default function WorkbenchPage2055() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center bg-slate-100 dark:bg-slate-950">
